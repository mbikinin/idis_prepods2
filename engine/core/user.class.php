...
class User {
     ...
     * @return bool
     */
  	public static function auth_simple($login, $pass) {
  		if(DB::fetch("SELECT user_login, 
  		user_password FROM user WHERE user_login = $login AND user_password = '".$pass."' AND user_activate = 1")){
  		Session::set("user_id", "$login");
  		return "OK";
  		}
  		elseif(DB::fetch("SELECT user_login, 
  		user_password FROM user WHERE user_login = $login AND user_password = '".$pass."' AND user_activate = 0")){
  		return "NO_FINISH";
  		}
  		else {
  		return "ERROR";
    ...
    public static function auth () {
       ...
       self::$_params=call_user_func(array('Auth', 'auth_ls'.self::$_backend));
